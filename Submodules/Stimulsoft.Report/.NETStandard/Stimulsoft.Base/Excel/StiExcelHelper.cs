...
namespace Stimulsoft.Base.Excel
{
    public static class StiExcelHelper
    {
        ...
        private static DataSet GetDataSetFromExcelDocument(Stream stream, bool firstRowIsHeader = true)
        {
            ...
            var hashCode = CalculateHashCode(stream);

            if (AllowCache)
            {
                lock (lockOjbect)
                {
                    var cacheDataSet = cache[hashCode] as DataSet;
                    if (cacheDataSet != null)
                        return cacheDataSet.Copy();
                }
            }
            ...
            if (buf[0] == 0x50 && buf[1] == 0x4B && buf[2] == 0x03 && buf[3] == 0x04)
                dataSet = GetDataSetFromXlsx(stream, firstRowIsHeader);
            else
                dataSet = GetDataSetFromXlsAlternate(stream, firstRowIsHeader);
        ...
        private static DataSet GetDataSetFromXlsAlternate(Stream stream, bool firstRowIsHeader = true, bool withoutTypes = false)
        {
            ...
            var assembly = StiLibExcellHelper.LibExcellAssembly;

            if (assembly == null)
                throw new Exception("'LibExcel.dll' assembly not found.");
        ...
        private static string CalculateHashCode(Stream stream)
        {
            if (!AllowCache)
                return null;
            ...
            try
            {
                var count = Math.Min(5000, stream.Length);
                var bytes = new byte[count];

                stream.Read(bytes, 0, (int)count);
                return BytesToString(SHA256.Create().ComputeHash(bytes)) + stream.Length.ToString();
            }
            finally
            {
                stream.Seek(0, SeekOrigin.Begin);
