...
namespace Stimulsoft.Base.Excel
{
	...
	public class ExcelOpenXmlReader : IExcelDataReader
	{
        ...
        private CultureInfo enCulture = CultureInfo.GetCultureInfo("en-US");

		private List<int> _defaultDateTimeStyles;
		private List<int> _defaultTimeStyles;
		#endregion
		...
		internal ExcelOpenXmlReader()
		{
			...
			_defaultTimeStyles = new List<int>(new int[]
			{
		...
		private void CheckDateTimeNumFmts(List<XlsxNumFmt> list)
		{
			...
			foreach (XlsxNumFmt numFmt in list)
			{
				...
				if (formatReader.IsDateFormatString())
				{
					...
					if (!(fc.Contains("y") || fc.Contains("mmm"))) _defaultTimeStyles.Add(numFmt.Id);
				}
			}
		}
...
//					}
		...
		private bool ReadSheetRow(XlsxWorksheet sheet)
		{
                ...
                _xmlReader.ReadToFollowing(XlsxWorksheet.N_row))
			{
                ...
                string _rowIndex = _xmlReader.GetAttribute(XlsxWorksheet.A_r);
                if (!string.IsNullOrWhiteSpace(_rowIndex))
                {
                    int rowIndex = int.Parse(_rowIndex);
                ...
                while (_xmlReader.Read())
                {
                    ...
                    if (_xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        if (_xmlReader.Name == XlsxWorksheet.N_c)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(a_r))
                            {
                        ...
                        else if (_xmlReader.Name == XlsxWorksheet.N_v || (_xmlReader.Name == XlsxWorksheet.N_t && prevNodeName == XlsxWorksheet.N_is))
                        {
                            hasValue = true;
                        }
                    }
                    ...
                    if (_xmlReader.NodeType == XmlNodeType.Text && hasValue)
                    {
                        ...
                        if (double.TryParse(o.ToString(), NumberStyles.Number | NumberStyles.AllowExponent , enCulture, out number))
							o = number;
                        ...
                        else if (null != a_s) //if something else
                        {
							...
							{
								//Import TimeSpan данных из Excel #4605
								if ((number >= 0.0 && number < 1.0) || (IsTimeStyle(xf.NumFmtId) && number >= 0.0 && number < 1000))
								{
									o = TimeSpan.FromDays(number);
								}
								else
								{
		...
		private bool IsTimeStyle(int styleId)
		{
			return _defaultTimeStyles.Contains(styleId);
		}
