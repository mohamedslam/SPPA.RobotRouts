...
namespace Stimulsoft.Base.Excel
{
    internal class XlsxWorkbook
    {
        ...
        private const string N_rPh = "rPh";
        ...
        private void ReadSharedStrings(Stream xmlFileStream)
        {
            ...
            using (XmlReader reader = XmlReader.Create(xmlFileStream))
            {
                ...
                while (!reader.EOF)
                {
                    if (reader.NodeType == XmlNodeType.Element && reader.Name == N_rPh)
                    {
                        while (!(reader.NodeType == XmlNodeType.EndElement && reader.Name == N_rPh) && !reader.EOF)
                        {
                            reader.Read();
                        }
                    }
