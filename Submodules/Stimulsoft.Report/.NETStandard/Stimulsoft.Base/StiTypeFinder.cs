    ...
    public sealed class StiTypeFinder
    {
        ...
        public static Type GetType(string typeName)
        {
            lock (TypeToString)
            {
                ...
                {
                    var type = StringToType[typeName] as Type;
                    if (type != null) return type;

#if STIDRAWING
                    if (typeName.IndexOf("System.Drawing") == 0)
                        typeName = typeName.Replace("System.Drawing", "Stimulsoft.Drawing");
#endif
                ...
                TypeNotFound?.Invoke(typeName, e);
        ...
        private static void AddTypeFF(Type exType, Type typeForFinding, bool result)
        {
            lock (FindTypes)
            {
                ...
                var ff = FindTypes[exType] as Hashtable;
                if (ff == null)
                {
                    ff = new Hashtable();
                    FindTypes[exType] = ff;
                ...
                if (!ff.ContainsKey(typeForFinding))
                    ff[typeForFinding] = result;
            }
        }
        ...
        public static bool FindInterface(Type exType, Type interfaceForFinding)
        {
            ...
            while (exType != typeof(object))
            {
                ...
                var interfaces = exType.GetInterfaces();
