...
namespace Stimulsoft.Base
{
    public class StiMySqlDevartConnector : StiMySqlConnector
    {
        ...
        /// </summary>
        public override Type SqlType => typeof(StiDbType.Devart.MySql);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.Devart.MySql.Text;

        /// <summary>
        /// Gets the package identificator for this connector.
        /// </summary>
        public override string[] NuGetPackages => new string[] { "dotConnect.Express.for.MySQL" };

        /// <summary>
        /// Gets the package version for this connector.
        /// </summary>
        public override string NuGetVersion => "8.8.862";
        #endregion
        ...
        public override StiDataSchema RetrieveSchema(bool allowException = false)
        {
            ...
            try
            {
                using (var connection = CreateConnection())
                {
                    ...
                    try
                    {
                        var tables = GetSchema(connection, "Tables", new[] { connection.Database, null });
                    ...
                    try
                    {
                        var views = GetSchema(connection, "Views", new[] { connection.Database, null });
                    ...
                    try
                    {
                        ...
                        {
                            var columns = GetSchema(connection, "Columns", new[] { connection.Database, table.Name, null });
                    ...
                    try
                    {
                        var procedures = GetSchema(connection, "Procedures", new[] { connection.Database, null });
                        foreach (var row in StiSchemaRow.All(procedures))
                        {
                            var procedure = StiDataTableSchema.NewProcedure(row.NAME, this);
                    ...
                    try
                    {
                        foreach (var schemaTable in schema.Tables)
                        {
                            using (var dataSet = new DataSet())
                            {
                                ...
                                {
                                    FillAdapter(adapter, dataSet, schemaTable.Name);
        ...
        public override DbConnection CreateConnection()
        {
            ...
            prop = connection.GetType().GetProperty("Unicode");
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime))
                return (int)StiDbType.Devart.MySql.DateTime;

            if (type == typeof(TimeSpan))
                return (int)StiDbType.Devart.MySql.TimeStamp;

            if (type == typeof(Int64))
                return (int)StiDbType.Devart.MySql.BigInt;

            if (type == typeof(Int32))
                return (int)StiDbType.Devart.MySql.Int;

            if (type == typeof(Int16))
                return (int)StiDbType.Devart.MySql.SmallInt;

            if (type == typeof(SByte))
                return (int)StiDbType.Devart.MySql.TinyInt;
            ...
            if (type == typeof(Single))
                return (int)StiDbType.Devart.MySql.Float;

            if (type == typeof(Double))
                return (int)StiDbType.Devart.MySql.Double;

            if (type == typeof(Decimal))
                return (int)StiDbType.Devart.MySql.Decimal;

            if (type == typeof(String))
                return (int)StiDbType.Devart.MySql.VarChar;

            if (type == typeof(Boolean))
                return (int)StiDbType.Devart.MySql.SmallInt;

            if (type == typeof(Char))
                return (int)StiDbType.Devart.MySql.Char;

            if (type == typeof(Byte[]))
                return (int)StiDbType.Devart.MySql.Binary;

            if (type == typeof(Guid))
                return (int)StiDbType.Devart.MySql.Guid;
        ...
        {
            return StiDataOptions.SampleConnectionString.MySqlDevart;
        }
        #endregion
