...
namespace Stimulsoft.Base
{
    public class StiFirebirdConnector : StiSqlDataConnector
    {
        ...
        /// </summary>
        public override Type SqlType => typeof(StiDbType.Firebird);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.Firebird.Text;

        /// <summary>
        /// Gets the package identificator for this connector.
        /// </summary>
        public override string[] NuGetPackages => new string[] { "FirebirdSql.Data.FirebirdClient" };

        /// <summary>
        /// Gets the package version for this connector.
        /// </summary>
        public override string NuGetVersion => "5.9.0.1";
        #endregion
        ...
        public override StiDataSchema RetrieveSchema(bool allowException = false)
        {
            ...
            try
            {
                using (var connection = CreateConnection())
                {
                    ...
                    try
                    {
                        var tables = GetSchema(connection, "Tables", new[] { null, null, null, "TABLE" });
                    ...
                    try
                    {
                        ...
                        {
                            var view = StiDataTableSchema.NewView(row.VIEW_NAME, this);
                    ...
                    try
                    {
                        ...
                        {
                            var procedure = StiDataTableSchema.NewProcedure(row.PROCEDURE_NAME, this);
                    ...
                    foreach (var procedure in schema.StoredProcedures)
                    {
                        try
                        {
                            var parameters = GetSchema(connection, "procedureparameters", new[] { null, null, procedure.Name });
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime)) 
                return (int)StiDbType.Firebird.Date;

            if (type == typeof(TimeSpan)) 
                return (int)StiDbType.Firebird.TimeStamp;

            if (type == typeof(Int64)) 
                return (int)StiDbType.Firebird.BigInt;

            if (type == typeof(Int32))
                return (int)StiDbType.Firebird.Integer;

            if (type == typeof(Int16)) 
                return (int)StiDbType.Firebird.SmallInt;
            ...
            if (type == typeof(Single)) 
                return (int)StiDbType.Firebird.Float;

            if (type == typeof(Double)) 
                return (int)StiDbType.Firebird.Double;

            if (type == typeof(Decimal)) 
                return (int)StiDbType.Firebird.Decimal;
            ...
            if (type == typeof(Char))
                return (int)StiDbType.Firebird.Char;

            if (type == typeof(Byte[])) 
                return (int)StiDbType.Firebird.Binary;

            if (type == typeof(Guid)) 
                return (int)StiDbType.Firebird.Guid;
        ...
        {
            return StiDataOptions.SampleConnectionString.Firebird;
        }
        #endregion
