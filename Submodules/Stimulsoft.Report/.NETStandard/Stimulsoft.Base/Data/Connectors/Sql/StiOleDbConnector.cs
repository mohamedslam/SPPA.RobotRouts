...
namespace Stimulsoft.Base
{
    public class StiOleDbConnector : StiSqlDataConnector
    {
        ...
        /// </summary>
        public override StiConnectionIdent ConnectionIdent => StiConnectionIdent.OleDbDataSource;

        /// <summary>
        /// Gets an order of the connector.
        /// </summary>
        public override StiConnectionOrder ConnectionOrder => StiConnectionOrder.OleDbDataSource;

        public override string Name => "OLE DB";
        ...
        /// </summary>
        public override Type ConnectionType => typeof(OleDbConnection);

        /// <summary>
        /// Gets the type of an enumeration which describes data types.
        /// </summary>
        public override Type SqlType => typeof(StiDbType.OleDb);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.OleDb.Variant;
        #endregion
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime)) 
                return (int)StiDbType.OleDb.DBDate;

            if (type == typeof(TimeSpan)) 
                return (int)StiDbType.OleDb.DBTimeStamp;

            if (type == typeof(Int64)) 
                return (int)StiDbType.OleDb.BigInt;

            if (type == typeof(Int32)) 
                return (int)StiDbType.OleDb.Integer;

            if (type == typeof(Int16)) 
                return (int)StiDbType.OleDb.SmallInt;

            if (type == typeof(Byte)) return (int)StiDbType.OleDb.TinyInt;

            if (type == typeof(UInt64))
                return (int)StiDbType.OleDb.UnsignedBigInt;

            if (type == typeof(UInt32))
                return (int)StiDbType.OleDb.UnsignedInt;

            if (type == typeof(UInt16)) 
                return (int)StiDbType.OleDb.UnsignedSmallInt;

            if (type == typeof(SByte)) 
                return (int)StiDbType.OleDb.UnsignedTinyInt;

            if (type == typeof(Single)) 
                return (int)StiDbType.OleDb.Single;

            if (type == typeof(Double)) 
                return (int)StiDbType.OleDb.Double;

            if (type == typeof(Decimal)) 
                return (int)StiDbType.OleDb.Decimal;

            if (type == typeof(String)) 
                return (int)StiDbType.OleDb.BSTR;

            if (type == typeof(Boolean)) 
                return (int)StiDbType.OleDb.Boolean;
            
            if (type == typeof(Char)) 
                return (int)StiDbType.OleDb.Char;
            
            if (type == typeof(Byte[])) 
                return (int)StiDbType.OleDb.Binary;
            
            if (type == typeof(Guid)) 
                return (int)StiDbType.OleDb.Guid;
        ...
        {
            return StiDataOptions.SampleConnectionString.OleDb;
        }
