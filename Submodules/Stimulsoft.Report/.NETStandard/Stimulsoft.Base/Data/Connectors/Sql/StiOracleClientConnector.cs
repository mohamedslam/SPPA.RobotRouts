...
namespace Stimulsoft.Base
{
    public class StiOracleClientConnector : StiOracleConnector
    {
        ...
        /// </summary>
        public override Type SqlType => typeof(StiDbType.OracleClient);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.OracleClient.NVarChar;
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime)) 
                return (int)StiDbType.OracleClient.DateTime;

            if (type == typeof(TimeSpan))
                return (int)StiDbType.OracleClient.Timestamp;

            if (type == typeof(Int64)) 
                return (int)StiDbType.OracleClient.Number;

            if (type == typeof(Int32)) 
                return (int)StiDbType.OracleClient.Int32;

            if (type == typeof(Int16)) 
                return (int)StiDbType.OracleClient.Int16;

            if (type == typeof(SByte)) 
                return (int)StiDbType.OracleClient.SByte;
            ...
            if (type == typeof(Byte)) 
                return (int)StiDbType.OracleClient.Byte;

            if (type == typeof(Single)) 
                return (int)StiDbType.OracleClient.Float;

            if (type == typeof(Double)) 
                return (int)StiDbType.OracleClient.Double;

            if (type == typeof(Decimal)) 
                return (int)StiDbType.OracleClient.Number;

            if (type == typeof(String)) 
                return (int)StiDbType.OracleClient.VarChar;

            if (type == typeof(Boolean)) 
                return (int)StiDbType.OracleClient.Byte;
            
            if (type == typeof(Char)) 
                return (int)StiDbType.OracleClient.Char;
            
            if (type == typeof(Byte[])) 
                return (int)StiDbType.OracleClient.Blob;
            
            if (type == typeof(Guid)) 
                return (int)StiDbType.OracleClient.Raw;
