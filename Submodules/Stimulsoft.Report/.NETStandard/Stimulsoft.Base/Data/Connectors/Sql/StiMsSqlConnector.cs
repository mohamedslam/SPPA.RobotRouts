...
namespace Stimulsoft.Base
{
    public class StiMsSqlConnector : StiSqlDataConnector
    {
        ...
        /// </summary>
        public override StiConnectionIdent ConnectionIdent => StiConnectionIdent.MsSqlDataSource;

        /// <summary>
        /// Gets an order of the connector.
        /// </summary>
        public override StiConnectionOrder ConnectionOrder => StiConnectionOrder.MsSqlDataSource;

        public override string Name => "MS SQL";
        ...
        /// </summary>
        public override Type SqlType => typeof(StiDbType.MsSql);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.MsSql.Text;
        #endregion
                ...
                using (var connection = CreateConnection())
                {
                    ...
                    try
                    {
                        ...
                        foreach (var row in StiSchemaRow.All(tables))
                        {
                            ...
                            var query = StiTableQuery.Get(this)
                                .GetSelectQuery(row.TABLE_SCHEMA != "dbo" ? row.TABLE_SCHEMA : null, row.TABLE_NAME);
                    ...
                    try
                    {
                        ...
                        var connectionDatabase = connection.Database?.ToUpperInvariant();
                    ...
                    try
                    {
                        ...
                        using (var commandGetParams = CreateCommand(queryGetParams, connection))
                        using (var reader = ExecuteReader(commandGetParams))
                        ...
                        #region Fill Columns
                        if (StiDataOptions.WizardStoredProcRetrieveMode == StiWizardStoredProcRetrieveMode.All)
                        {
                            try
                            {
                                foreach (var procedure in schema.StoredProcedures)
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime))
                return (int)StiDbType.MsSql.DateTime;

            if (type == typeof(DateTimeOffset))
                return (int)StiDbType.MsSql.DateTimeOffset;

            if (type == typeof(TimeSpan))
                return (int)StiDbType.MsSql.Timestamp;

            if (type == typeof(Int64))
                return (int)StiDbType.MsSql.BigInt;

            if (type == typeof(Int32))
                return (int)StiDbType.MsSql.Int;

            if (type == typeof(Int16))
                return (int)StiDbType.MsSql.SmallInt;

            if (type == typeof(Byte))
                return (int)StiDbType.MsSql.TinyInt;
            ...
            if (type == typeof(Single))
                return (int)StiDbType.MsSql.Float;

            if (type == typeof(Double))
                return (int)StiDbType.MsSql.Real;

            if (type == typeof(Decimal))
                return (int)StiDbType.MsSql.Decimal;

            if (type == typeof(String))
                return (int)StiDbType.MsSql.VarChar;

            if (type == typeof(Boolean))
                return (int)StiDbType.MsSql.Bit;

            if (type == typeof(Char))
                return (int)StiDbType.MsSql.Char;

            if (type == typeof(Byte[]))
                return (int)StiDbType.MsSql.Binary;

            if (type == typeof(Guid))
                return (int)StiDbType.MsSql.UniqueIdentifier;
        ...
        {
            return StiDataOptions.SampleConnectionString.MsSql;
        }
