...
namespace Stimulsoft.Base
{
    public class StiOdbcConnector : StiSqlDataConnector
    {
        ...
        /// </summary>
        public override StiConnectionIdent ConnectionIdent => StiConnectionIdent.OdbcDataSource;

        /// <summary>
        /// Gets an order of the connector.
        /// </summary>
        public override StiConnectionOrder ConnectionOrder => StiConnectionOrder.OdbcDataSource;
        ...
        /// </summary>
        public override Type ConnectionType => typeof(OdbcConnection);

        /// <summary>
        /// Gets the type of an enumeration which describes data types.
        /// </summary>
        public override Type SqlType => typeof(StiDbType.Odbc);

        /// <summary>
        /// Gets the default value of the data parameter.
        /// </summary>
        public override int DefaultSqlType => (int)StiDbType.Odbc.Text;
        #endregion
        ...
        public override StiDataSchema RetrieveSchema(bool allowException = false)
        {
            ...
            try
            {
                using (var connection = CreateConnection())
                {
                    ...
                    var option = GetSchemaColumnsMode;
                    ...
                    try
                    {
                        ...
                        {
                            var schemaName = (option ? row[1].ToString() : row["TABLE_SCHEM"].ToString());
                            var tableName = (option ? row[2].ToString() : row["TABLE_NAME"].ToString());
                            if (schemaName == "sys") continue;

                            if (tableName.Trim().Contains(" ")) tableName = string.Format("[{0}]", tableName);
                            tableName = StiTableName.GetName(schemaName != "dbo" ? schemaName : null, tableName);
                            var query = string.Format("select * from {0}{1}", (schemaName == "dbo" || schemaName == "" ? "" : string.Format("{0}.", schemaName)), tableName);
                    ...
                    try
                    {
                        if (AdvancedRetrievalModeOfDatabaseSchema)
                        {
                            ...
                            foreach (DataRow row in tables.Rows)
                            {
                                ...
                                var query = string.Format("select * from {0}", (tableName.Trim().Contains(" ") ? string.Format("[{0}]", tableName) : tableName));
                                ...
                                using (var table = reader.GetSchemaTable())
                                {
                                    ...
                                    {
                                        var columnSchema = new StiDataColumnSchema(rowTable["ColumnName"].ToString(), Type.GetType(rowTable["DataType"].ToString()));
                                        tableName = StiTableName.GetName(schemaName != "dbo" ? schemaName : null, tableName);
                                        var tableSchema = tableHash[tableName] as StiDataTableSchema;
                                        if (tableSchema != null && !tableSchema.Columns.Exists(t => t.Name == rowTable["ColumnName"].ToString()))
        ...
        public override int GetSqlType(Type type)
        {
            if (type == typeof(DateTime)) 
                return (int)StiDbType.Odbc.DateTime;

            if (type == typeof(TimeSpan)) 
                return (int)StiDbType.Odbc.Timestamp;

            if (type == typeof(Int64)) 
                return (int)StiDbType.Odbc.BigInt;

            if (type == typeof(Int32)) 
                return (int)StiDbType.Odbc.Int;

            if (type == typeof(Int16)) 
                return (int)StiDbType.Odbc.SmallInt;

            if (type == typeof(Byte)) 
                return (int)StiDbType.Odbc.TinyInt;
            ...
            if (type == typeof(Single)) 
                return (int)StiDbType.Odbc.Decimal;

            if (type == typeof(Double)) 
                return (int)StiDbType.Odbc.Double;
            ...
            if (type == typeof(Boolean))
                return (int)StiDbType.Odbc.Char;

            if (type == typeof(Char)) 
                return (int)StiDbType.Odbc.Char;

            if (type == typeof(Byte[])) 
                return (int)StiDbType.Odbc.Binary;

            if (type == typeof(Guid)) 
                return (int)StiDbType.Odbc.UniqueIdentifier;
        ...
        {
            return StiDataOptions.SampleConnectionString.Odbc;
        }
            ...
            : base(connectionString)
        {
            ...
            this.TypeConnection = typeof(OdbcConnection).ToString();
            this.TypeDataAdapter = typeof(OdbcDataAdapter).ToString();
            this.TypeCommand = typeof(OdbcCommand).ToString();
            this.TypeParameter = typeof(OdbcParameter).ToString();
            this.TypeCommandBuilder = typeof(OdbcCommandBuilder).ToString();
        }
    }
}
