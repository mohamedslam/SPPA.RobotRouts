...
namespace Stimulsoft.Base
{
    public class StiDBaseHelper
    {
		...
		{
			return GetTable(path, 0, maxDataRows);
		}

		/// <summary>
        /// Convert table from the DBase file.
		/// </summary>
        public static DataTable GetTable(string path, int codePage, int maxDataRows = 0)
		{
		    return GetTable(path, codePage != 0, codePage, maxDataRows);
		}

        /// <summary>
        /// Convert table from the DBase file.
        /// </summary>
        private static DataTable GetTable(string path, bool forceCodePage, int forcedCodePage, int maxDataRows = 0)
        {
            DataTable dt = null;
            using (Stream data = new FileStream(path, FileMode.Open))
            {
                ...
                if (File.Exists(memoPath))
                {
                    ...
                    {
                        dt = GetTable(data, memo, forceCodePage, forcedCodePage, maxDataRows);
                    }
                }
                else
                {
                    dt = GetTable(data, null, forceCodePage, forcedCodePage, maxDataRows);
                }
            }
            return dt;
        }
        ...
        /// </summary>
        public static DataSet GetDataSet(byte[] data, string tableName, bool forceCodePage, int forcedCodePage, int maxDataRows = 0)
        {
            DataTable dataTable = null;
            using (var sdata = new MemoryStream(data))
            {
                dataTable = GetTable(sdata, null, forceCodePage, forcedCodePage, maxDataRows);
            }

            if (dataTable == null) 
                return null;

            dataTable.TableName = string.IsNullOrWhiteSpace(tableName) ? StiFileItemTable.DefaultDBaseTableName : tableName;
        ...
        /// </summary>
        public static DataTable GetTable(byte[] data, int codePage, int maxDataRows = 0)
        {
            using (var sdata = new MemoryStream(data))
            {
                return GetTable(sdata, null, codePage != 0, codePage, maxDataRows);
            }
        }
        ...
        /// </summary>
        public static DataTable GetTable(Stream memoryStream, Stream memo, bool forceCodePage, int forcedCodePage, int maxDataRows = 0)
        {
            ...
            //using (var memoryStream = new MemoryStream(data))
            {
                ...
                var codePage = 0;
                for (var indexCP = 0; indexCP < CodePageCodes.GetLength(0); indexCP++)
                {
