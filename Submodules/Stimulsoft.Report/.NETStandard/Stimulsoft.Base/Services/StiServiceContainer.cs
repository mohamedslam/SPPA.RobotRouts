...
namespace Stimulsoft.Base.Services
{
	...
	public class StiServiceContainer : CollectionBase, ICloneable
	{
		...
		#region Properties
	    public static string[] StandardAssemblies { get; set; } =
		{
		    "Stimulsoft.Base.dll",
		...
		public StiServiceContainer GetServices(Type serviceType, bool getEnabled, bool callBeforeGetService)
		{
			...
			if (!noTypes)
			{
				var services = typesHashtable[serviceType] as StiServiceContainer;
				if (services != null)
				{
					var serv = new StiServiceContainer();
                    lock (services)
                    {
                        foreach (StiService service in services)
                        {
			...
			{
				var services = new StiServiceContainer(true);
		...
		public static string GetStringFromService(Type serviceType)
		{
			...
			var ext = serviceType.ToString().StartsWithInvariant("Stimulsoft.") 
                ? ".dll" 
                : Path.GetExtension(assembly.CodeBase).ToLowerInvariant();

			return $"{assembly.GetName().Name}{ext},{serviceType}";
		}
	    ...
	    private static Type[] GetTypesFromAssembly(Assembly a)
		{
			...
			{
				var typesInAssembly = AssemblyToTypes[a] as Type[];
			...
			while (tr.Read())
			{
				if (tr.IsStartElement()) 
				{
					...
					if (tr.Name == "service")
					{
						...
						if (assemlyToSkip[assembly.ToLower(CultureInfo.InvariantCulture)] == null)
						{
