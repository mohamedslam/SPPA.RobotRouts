...
namespace Stimulsoft.Report.Web
{
    internal class StiFontsHelper
    {
        ...
        public static ArrayList GetCustomOpenTypeFontItems(bool allowLoadingCustomFonts)
        {
            var families = StiFontCollection.GetFontFamilies().ToArray();
            var items = new ArrayList();
            ...
            foreach (var fontFamily in families)
            {
                ...
                if (allowLoadingCustomFonts && !string.IsNullOrEmpty(fontName) && StiFontCollection.IsCustomFont(fontName))
                {
                    ...
                    if (fontData != null)
                    {
                        ...
                        {
                            ["originalFontFamily"] = fontName,
                            ["contentForCss"] = $"{GetFontMimeType(fontData)};base64,{Convert.ToBase64String(fontData)}"
                        });
                    }
