...
{
    public class StiChartSeriesValueCheck : StiComponentCheck
    {
        #region Properties
        ...
        public override string LongMessage
        {
            get
            {
                return string.Format(StiLocalizationExt.Get("CheckComponent", "StiChartSeriesValueCheckLong"), this.ElementName);
            }
        }
        ...
        private bool Check()
        {
            var chart = Element as StiChart;
            if (chart != null && chart.DataSource == null && chart.Series.Count > 0)
            {
                foreach (StiSeries ser in chart.Series)
                {
                    if (!string.IsNullOrEmpty(ser.Value.Value))
                    {
                        return true;
                    }
                }
        ...
        public override object ProcessCheck(StiReport report, object obj)
        {
            ...
            try
            {
                ...
                if (failed)
                {
                    var check = new StiChartSeriesValueCheck();
                    check.Element = obj;
                    return check;
                }
                else return null;
