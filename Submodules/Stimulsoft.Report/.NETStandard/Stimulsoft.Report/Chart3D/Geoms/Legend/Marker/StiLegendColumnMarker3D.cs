...
{
    public class StiLegendColumnMarker3D : IStiLegendMarker
    {
        public void Draw(StiContext context, IStiSeries series, RectangleF rect, int colorIndex, int colorCount, int index)
        {
            try
            {
                var series3D = series as IStiClusteredColumnSeries3D;
                var color = ((StiSeriesCoreXF3D)series.Core).GetSeriesColor(colorIndex, colorCount, series3D.Color);
                color = series.ProcessSeriesColors(colorIndex, color);
                var br = new StiSolidBrush(color);
