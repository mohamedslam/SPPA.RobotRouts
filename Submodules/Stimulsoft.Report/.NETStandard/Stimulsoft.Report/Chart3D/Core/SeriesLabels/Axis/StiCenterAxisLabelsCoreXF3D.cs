...
{
    public class StiCenterAxisLabelsCoreXF3D : StiAxisSeriesLabelsCoreXF3D
    {        
        #region Properties
        ...
        /// Gets a service name.
        /// </summary>
        public override string LocalizedName => StiLocalization.Get("Chart", "LabelsCenter");
        #endregion
            ...
            int colorIndex, int colorCount)
        {
            ...
            var point3D = GetPointLabel(rect3D);

            return new StiCenterAxisLabelsGeom3D(this.SeriesLabels, series, pointIndex, value.Value, labelText, labelColor, labelBorderColor,
                seriesBrush, seriesLabelsBrush, seriesBorderColor, font, point3D, render3D);
        }

        private StiPoint3D GetPointLabel(StiRectangle3D rect3D)
        {
            return new StiPoint3D(rect3D.X + rect3D.Length / 2, rect3D.Y + rect3D.Height / 2, rect3D.Front);
        }

        public StiCenterAxisLabelsCoreXF3D(IStiSeriesLabels seriesLabels)
            : base(seriesLabels)
        {
        }
    }
