...
{
	public class StiPageFooterBandV1Builder : StiBandV1Builder
	{
		...
		{
			StiPageFooterBand masterPageFooter = masterComp as StiPageFooterBand;

			if (!StiHeaderBandV1Builder.PrintOddEven(masterPageFooter, masterPageFooter.PrintOnEvenOddPages))return true;

			foreach (StiComponent comp in masterPageFooter.Components)
			{
				if (comp.IsCross)comp.Prepare();
			}
			bool result = base.InternalRender(masterPageFooter, ref renderedComponent, outContainer);
