...
{
    public class StiLeftAndRightLineGdiShapeTypePainter : StiGdiShapeTypePainter
    {
        ...
        public override void Paint(object context, StiShape shape, StiShapeTypeService shapeType, RectangleF rect, float zoom)
        {
            ...
            rect = GetRectLeftRight(shape, rect, zoom);
            ...
            if (shape.Style != StiPenStyle.None)
            {
                ...
                using (var pen = new Pen(shape.BorderColor, shape.Style == StiPenStyle.Double ? offsetDouble : shape.Size * zoom))
                {
                    ...
                    {
                        g.DrawLine(pen, rect.X - offsetDouble, rect.Y, rect.X - offsetDouble, rect.Bottom);
                        g.DrawLine(pen, rect.X + offsetDouble, rect.Y, rect.X + offsetDouble, rect.Bottom);
                        g.DrawLine(pen, rect.Right - offsetDouble, rect.Y, rect.Right - offsetDouble, rect.Bottom);
                        g.DrawLine(pen, rect.Right + offsetDouble, rect.Y, rect.Right + offsetDouble, rect.Bottom);
                    }
                    else
                    {
