...
{
    public class StiTopAndBottomLineGdiShapeTypePainter : StiGdiShapeTypePainter
    {
        ...
        public override void Paint(object context, StiShape shape, StiShapeTypeService shapeType, RectangleF rect, float zoom)
        {
            ...
            rect = GetRectTopBottom(shape, rect, zoom);
            ...
            if (shape.Style != StiPenStyle.None)
            {
                ...
                using (var pen = new Pen(shape.BorderColor, shape.Style == StiPenStyle.Double ? offsetDouble : shape.Size * zoom))
                {
                    ...
                    {
                        g.DrawLine(pen, rect.X, rect.Y - offsetDouble, rect.Right, rect.Y - offsetDouble);
                        g.DrawLine(pen, rect.X, rect.Y + offsetDouble, rect.Right, rect.Y + offsetDouble);
                        g.DrawLine(pen, rect.X, rect.Bottom - offsetDouble, rect.Right, rect.Bottom - offsetDouble);
                        g.DrawLine(pen, rect.X, rect.Bottom + offsetDouble, rect.Right, rect.Bottom + offsetDouble);
                    }
                    else
                    {
