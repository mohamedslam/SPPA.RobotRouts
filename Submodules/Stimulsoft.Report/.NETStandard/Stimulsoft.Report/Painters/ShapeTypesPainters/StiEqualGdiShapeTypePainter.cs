...
{
    public class StiEqualGdiShapeTypePainter : StiGdiShapeTypePainter
    {
        ...
        public override void Paint(object context, StiShape shape, StiShapeTypeService shapeType, RectangleF rect, float zoom)
        {
            ...
            {
                float height = (rect.Height - (rect.Height/6))/2;

                path.AddRectangle(new RectangleF(rect.X, rect.Bottom - height, rect.Width, height));
                path.AddRectangle(new RectangleF(rect.X, rect.Y, rect.Width, height));
