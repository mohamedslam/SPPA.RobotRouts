...
{
    public class StiDiagonalUpLineGdiShapeTypePainter : StiGdiShapeTypePainter
    {
            ...
            if (shape.Style != StiPenStyle.None)
            {
                ...
                using (var pen = new Pen(shape.BorderColor, shape.Style == StiPenStyle.Double ? offsetDouble : shape.Size * zoom))
                {
                    ...
                    if (shape.Style == StiPenStyle.Double)
                    {
                        ...
                        g.DrawLine(pen, rect.X, rect.Bottom - offset, rect.Right - offset, rect.Y);
                        g.DrawLine(pen, rect.X + offset, rect.Bottom, rect.Right, rect.Y + offset);
                    }
                    else
                    {
