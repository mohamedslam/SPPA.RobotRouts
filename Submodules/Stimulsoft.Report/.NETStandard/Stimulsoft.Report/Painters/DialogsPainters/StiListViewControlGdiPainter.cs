...
{
    public class StiListViewControlGdiPainter : StiReportControlGdiPainter
    {
        ...
        public override void Paint(StiComponent component, StiPaintEventArgs e)
        {
            ...
            StiListViewControl listView = component as StiListViewControl;
            listView.InvokePainting(listView, e);

            if (!e.Cancel && (!(listView.Enabled == false && listView.IsDesigning == false)))
            {
                Graphics g = e.Graphics;

                RectangleD rect = listView.GetPaintRectangle();
                ...
                if (rect.Width > 0 && rect.Height > 0 && (e.ClipRectangle.IsEmpty || rect.IntersectsWith(e.ClipRectangle)))
                {
                    ...
                    using (SolidBrush brush = new SolidBrush(listView.ForeColor))
                    using (StringFormat sf = new StringFormat())
                    {
                        g.DrawString(listView.Name, listView.Font, brush, borderRect, sf);
                    }

                    PaintEvents(listView, e.Graphics, rect);
                    PaintConditions(listView, e.Graphics, rect);
                }
            }
            e.Cancel = false;
            listView.InvokePainted(listView, e);
        }
        #endregion
    }
}
