...
{
    public class StiStackedSplineAreaSeriesGeom : StiSeriesGeom
    {
        ...
        {
            IStiStackedSplineAreaSeries areaSeries = this.Series as IStiStackedSplineAreaSeries;
            var isAnimation = ((StiChart)areaSeries.Chart).IsAnimation;

            IStiAxisArea axisArea = this.Series.Chart.Area as IStiAxisArea;
            //RectangleF rect = area.ClientRectangle;
            //float posY = axisArea.GetDividerY();
            ...
            foreach (PointF[] newStartPoints in startList)
            {
                ...
                var lineSegments = new StiCurveSegmentGeom(newEndPoints, areaSeries.Tension);
                var lineSegment2 = new StiLineSegmentGeom(newEndPoints[newEndPoints.Length - 1], newStartPoints[newStartPoints.Length - 1]);
                var reverseLineSegment = new StiCurveSegmentGeom(reverseStartPoint, areaSeries.Tension);
        ...
        public StiStackedSplineAreaSeriesGeom(StiAreaGeom areaGeom, StiSeriesPointsInfo pointsInfo, IStiSeries series)
            : base(areaGeom, series, GetClientRectangle(pointsInfo.Points, pointsInfo.PointsEnd))
        {
