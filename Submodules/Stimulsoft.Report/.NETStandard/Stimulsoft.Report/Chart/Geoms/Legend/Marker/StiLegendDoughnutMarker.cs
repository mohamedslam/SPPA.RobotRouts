...
{
    public class StiLegendDoughnutMarker : IStiLegendMarker
    {
        public void Draw(StiContext context, IStiSeries serie, RectangleF rect, int colorIndex, int colorCount, int index)
        {
            ...
            path.Add(new StiArcSegmentGeom(
                ...
                270, 90));
            path.Add(new StiLineSegmentGeom(rect.Right, rect.Bottom, rect.X + rect.Width / 2, rect.Bottom));

            path.Add(new StiArcSegmentGeom(
                new RectangleF(
                rect.X - rect.Width / 2, rect.Bottom - rect.Height / 2,
                rect.Width, rect.Height),
                360, -90));

            path.Add(new StiLineSegmentGeom(rect.X, rect.Y + rect.Height / 2, rect.X, rect.Y));
