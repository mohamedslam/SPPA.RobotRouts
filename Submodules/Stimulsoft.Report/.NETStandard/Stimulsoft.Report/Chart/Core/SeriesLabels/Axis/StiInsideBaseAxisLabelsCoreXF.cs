...
{
    public class StiInsideBaseAxisLabelsCoreXF : StiCenterAxisLabelsCoreXF
    {
        ...
        public override int Position
        {
            get
            {
                return (int)StiSeriesLabelsPosition.InsideBaseAxis;
            }
        }
        #endregion
            ...
            double? value, string labelText, bool checkHeight, StiFontGeom font, StiStringFormatGeom sf)
        {
            ...
            if (this.SeriesLabels.Chart.Area.Core.SeriesOrientation == StiChartSeriesOrientation.Vertical)
            {
                if (value > 0)
                {
                    var textPointY = startPoint.Y - (float)(Math.Abs(size.Height * Math.Cos(SeriesLabels.Angle * Math.PI / 180)) + Math.Abs((size.Width / 2 + deltaWidth) * Math.Sin(SeriesLabels.Angle * Math.PI / 180)));
                    if (textPointY < endPoint.Y)
