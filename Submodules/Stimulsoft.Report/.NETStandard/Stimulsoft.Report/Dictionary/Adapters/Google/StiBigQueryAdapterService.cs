...
{
    public class StiBigQueryAdapterService : StiNoSqlAdapterService
    {
        #region Methods
        /// <summary>
        ...
        {
            return typeof(StiBigQuerySource);
        }
        ...
        {
            return StiBigQueryConnector.Get(connectionString);
        }
        ...
        public override StiDataColumnsCollection GetColumnsFromData(StiData data, StiDataSource dataSource, CommandBehavior retrieveMode)
        {
            ...
            var source = dataSource as StiBigQuerySource;

            var connection = dataSource.Dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d.Name.ToLowerInvariant() == source.NameInSource.ToLowerInvariant()) as StiBigQueryDatabase;
        ...
        public override void ConnectDataSourceToData(StiDictionary dictionary, StiDataSource dataSource, bool loadData)
        {
            ...
            var bigQuerySource = dataSource as StiBigQuerySource;

            var database = dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d is StiBigQueryDatabase && d.Name.ToLowerInvariant() == bigQuerySource.NameInSource.ToLowerInvariant()) as StiBigQueryDatabase;
            if (database == null) return;
            var connector = CreateConnector(database.ConnectionString);

            dataSource.DataTable = connector.GetDataTable(bigQuerySource.Name, bigQuerySource.Query);
        }
        #endregion
    }
}
