...
{
    public class StiFirebaseAdapterService : StiNoSqlAdapterService
    {
        #region Methods
        /// <summary>
        ...
        {
            return typeof(StiFirebaseSource);
        }
        ...
        {
            return StiFirebaseConnector.Get(connectionString);
        }
        ...
        public override StiDataColumnsCollection GetColumnsFromData(StiData data, StiDataSource dataSource, CommandBehavior retrieveMode)
        {
            ...
            var source = dataSource as StiFirebaseSource;

            var connection = dataSource.Dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d.Name.ToLowerInvariant() == source.NameInSource.ToLowerInvariant()) as StiFirebaseDatabase;
        ...
        public override void ConnectDataSourceToData(StiDictionary dictionary, StiDataSource dataSource, bool loadData)
        {
            ...
            var firebaase = dataSource as StiFirebaseSource;

            var database = dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d is StiFirebaseDatabase && d.Name.ToLowerInvariant() == firebaase.NameInSource.ToLowerInvariant()) as StiFirebaseDatabase;
            if (database == null) return;
            var connector = CreateConnector(database.ConnectionString);

            dataSource.DataTable = connector.GetDataTable(firebaase.Name, "");
        }
        #endregion
    }
}
