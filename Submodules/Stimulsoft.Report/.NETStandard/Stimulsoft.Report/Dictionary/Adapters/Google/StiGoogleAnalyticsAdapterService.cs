...
{
    public class StiGoogleAnalyticsAdapterService : StiNoSqlAdapterService
    {
        #region Methods
        /// <summary>
        ...
        {
            return typeof(StiGoogleAnalyticsSource);
        }
        ...
        {
            return StiGoogleAnalyticsConnector.Get(connectionString);
        }
        ...
        public override StiDataColumnsCollection GetColumnsFromData(StiData data, StiDataSource dataSource, CommandBehavior retrieveMode)
        {
            ...
            var source = dataSource as StiGoogleAnalyticsSource;

            var connection = dataSource.Dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d.Name.ToLowerInvariant() == source.NameInSource.ToLowerInvariant()) as StiGoogleAnalyticsDatabase;
        ...
        public override void ConnectDataSourceToData(StiDictionary dictionary, StiDataSource dataSource, bool loadData)
        {
            ...
            var googleAnalyticsSource = dataSource as StiGoogleAnalyticsSource;

            var database = dictionary.Databases.Cast<StiDatabase>().FirstOrDefault(d => d is StiGoogleAnalyticsDatabase && d.Name.ToLowerInvariant() == googleAnalyticsSource.NameInSource.ToLowerInvariant()) as StiGoogleAnalyticsDatabase;
            if (database == null) return;
            var connector = CreateConnector(database.ConnectionString);

            dataSource.DataTable = connector.GetDataTable(googleAnalyticsSource.Name, googleAnalyticsSource.Query);
        }
        #endregion
    }
}
