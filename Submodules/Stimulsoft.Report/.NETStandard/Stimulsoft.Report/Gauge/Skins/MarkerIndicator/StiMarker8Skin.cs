...
{
    public class StiMarker8Skin : StiMarkerBaseSkin
    {
        public override void Draw(StiGaugeContextPainter context, StiGaugeElement element, RectangleF rect, float? angle, PointF? centerPoint)
        {
            ...
            float top = rect.Top + rect.Height / 5;
            float width = rect.Width / 3;

            var points = new PointF[7];
            points[0] = new PointF(rect.Left, top);
            points[1] = new PointF(rect.Left + width, top);
            points[2] = new PointF(rect.Left + rect.Width / 2, rect.Top);
            points[3] = new PointF(rgt - width, top);
            points[4] = new PointF(rgt, top);
            points[5] = new PointF(rgt, btm);
            points[6] = new PointF(rect.Left, btm);
